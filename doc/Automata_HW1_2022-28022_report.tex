...
\setmainfont[AutoFakeSlant]{NanumMyeongjo}
...
\begin{breakablealgorithm}
  ...
  \begin{algorithmic}[1]
    \Function{Automata::accepts}{$\Delta$, $s$}
      \State $S_1 \coloneq$ Set of current states
      \State $S_2 \coloneq$ Set of next states
      \State $S_1 \gets \{q_0\}$
      \State \Call{epsilon\_transitions}{$\Delta$, $S_1$}
      \For{$\textbf{each}\ a\ \textbf{in}\ s$}
        \State $S_2 \gets \bigcup\limits_{q_i \in Q} \Delta(q_i, a)$
        \lIf{$S_2 = \emptyset$}{\Return false}
        \State \Call{epsilon\_transitions}{$\Delta$, $S_2$}
        \State $S_1 \gets S_2$
      \EndFor
      \State \Return $q_{\mathrm{f}} \in S_1$
    \EndFunction
    \Statex
    \Function{epsilon\_transitions}{$\Delta$, $S$}
...
\begin{enumerate}
  \item 조건~\ref{cond:regex}\를 만족하는 regex를 생성하고,
  \item 적절한 입력 string을 랜덤하게 생성하며,
  \item 각 string이 regex에 의해 accept되는지 \texttt{grep} 유틸리티를 이용하여 정답을 작성한다.
\end{enumerate}
